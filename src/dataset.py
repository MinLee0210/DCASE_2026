...
from src.utils.basic_utils import load_jsonl, l2_normalize_np_array
...
class StartEndDataset(Dataset):
    ...
    def get_pos_mask(self, meta: Dict[str, Any], ctx_l: int) -> torch.Tensor:
        """
        Generate a positive mask for TR-DETR model indicating relevant clip positions.

        This method creates a binary mask where positions corresponding to relevant clips
        are set to 1, and others to 0. Used specifically for TR-DETR as model_inputs["pos_mask"].

        Args:
            meta (Dict[str, Any]): Metadata dictionary containing either "relevant_clip_ids"
                (list of int) or "relevant_windows" (list of [start, end] in seconds).
            ctx_l (int): Context length, i.e., total number of clips in the audio/video.

        Returns:
            torch.Tensor: Binary mask tensor of shape (ctx_l,) or extended if necessary,
                with 1s at relevant positions.

        Note:
            For certain datasets ("charades", "tacos", "activitynet", "clotho-moment",
            "unav100-subset", "tut2017"), the mask is trimmed to ctx_l.
        """
