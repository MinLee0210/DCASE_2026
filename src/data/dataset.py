...
class StartEndDataset(Dataset):
    ...
    def load_data(self) -> List[Dict[str, Any]]:
        ...
        valid_data = []
        try:
            a_files = set(os.listdir(self.a_feat_dir))
            q_files = set(os.listdir(self.q_feat_dir))
        except FileNotFoundError:
            a_files = set()
            q_files = set()
        for data in datalist:
            if f"{data['vid']}.npz" in a_files and f"qid{data['qid']}.npz" in q_files:
                valid_data.append(data)
        print(
            f"Loaded {len(valid_data)} valid samples out of {len(datalist)} total in {self.data_path}"
        )
        return valid_data
