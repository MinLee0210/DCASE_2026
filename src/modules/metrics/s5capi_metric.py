...
class S5ClassAwareMetric:
    def __init__(self, metricfunc="sdr"):
        if metricfunc == "sdr":
            ...
            raise ValueError(f"metricfunc of '{metricfunc}' is not implemented!!")
        self.metric_values = []
        self.reset()
    ...
    def compute(self, is_print=False):
        ...
        reobj = {"mean": sum(non_None_metric_values) / len(non_None_metric_values)}
        if is_print:
    ...
    def compute_batch(
        ...
        return [
            self.compute_sample(est_lb, est_wf, ref_lb, ref_wf, mixture)
    ...
    def _pi_metric(
        ...
        mixture_repeat = mixture.view(1, 1, -1).expand(
            perm_est_wfs_stack.shape[0], perm_est_wfs_stack.shape[1], -1
        )

        # calculate metric
        metrics = self.metric_func(
            perm_est_wfs_stack, perm_ref_wfs_stack
        )  # nperm, n_tp
        metrics_mixture = self.metric_func(
            mixture_repeat, perm_ref_wfs_stack
        )  # nperm, n_tp
        ...
        if self.min_max == "max":
            best_i = torch.argmax(metrics_mean).item()
        elif self.min_max == "min":
            best_i = torch.argmin(metrics_mean).item()
        else:
            raise NotImplementedError(
                f"min_max '{self.min_max}' has not been implemented."
            )
    ...
    def compute_sample(
        ...
        if not all_labels:
            return None  # true silence prediction
        ...
        est_dict = {
            lb: torch.stack(est_lists[lb], dim=0)
            if est_lists[lb]
            else torch.empty(
        ...
        ref_dict = {
            lb: torch.stack(ref_lists[lb], dim=0)
            if ref_lists[lb]
            else torch.empty(
        ...
        for lb in all_labels:
            ...
            if est_wf_1c.shape[0] == 0:  # all False Negative
                ...
                output = self._pi_metric(
                    est_wf=est_wf_1c, ref_wf=ref_wf_1c, mixture=mixture
                )
